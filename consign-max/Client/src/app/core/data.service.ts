...
export class DataService {
  ...
  getConsigners(): Observable<IConsigner[]>  {
    return this.http.get<IConsigner[]>(this.baseConsignersUrl)
    .pipe(
           map(consigners => {
  ...
  getConsignersPage(page: number, pageSize: number) : Observable<IPagedResults<IConsigner[]>> {
    return this.http.get<IConsigner[]>(`${this.baseConsignersUrl}/page/${page}/${pageSize}`, {observe: 'response'})
        .pipe(
            map((res) => {
                //Need to observe response in order to get to this header (see {observe: 'response'} above)
                const totalRecords = +res.headers.get('x-inlinecount');
                let consigners = res.body as IConsigner[];
                this.calculateConsignersAmountDue(consigners);
                return {
                    results: consigners,
  ...
  getConsigner(id: string) : Observable<IConsigner> {
    return this.http.get<IConsigner>(this.baseConsignersUrl + '/' + id)
        .pipe(
            catchError(this.handleError)
        );
}
...
insertConsigner(consigner: IConsigner) : Observable<IConsigner> {
    return this.http.post<IConsignerResponse>(this.baseConsignersUrl, consigner)
        .pipe(
            map((data) => {
...
updateConsigner(consigner: IConsigner) : Observable<IConsigner> {
    return this.http.put<IConsignerResponse>(this.baseConsignersUrl + '/' + consigner.id, consigner)
        .pipe(
            map((data) => {
...
deleteConsigner(id: string) : Observable<boolean> {
    return this.http.delete<boolean>(this.baseConsignersUrl + '/' + id)
        .pipe(
            catchError(this.handleError)
        );
}
